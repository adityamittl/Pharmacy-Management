let discountMoney = 0;
            ...
            fetch("/api/bill", {
                ...
                .then(data => {
                    ...
                    if(data.status == 'StockOut'){
                        alert(`Item ${data.med} is not sufficient in stock\nPlease reduce the quantity\nCurrent stock is ${data.stock}`);
                    }
                })
        }

        const syncTotal = () => {
            let ttl = Number(document.getElementById('subtotal').innerHTML) - Number(document.getElementById('discounted').value);
            if (ttl < 0) {
                ttl = 0;
            }
        ...
        const NewDiscount = () => {
            ...
            let newprice = Number(document.getElementById("subtotal").innerHTML) - Number(newDiscount);
        ...
        const DiscountedPrice = () => {
            ...
            let discounted = (total * discount) / 100;
            discountMoney = discounted;
            document.getElementById('discounted').value = discounted;
            document.getElementById('total').innerHTML = total - discounted;
        }
        ...
        const changequantity = (tab, medNo) => {
            ...
            const medprice = document.getElementById(`row-${medNo}`).childNodes[2].innerHTML;
            console.log(medprice)
            const total = medprice * medqty;
            document.getElementById(`row-${medNo}`).childNodes[3].innerHTML = total;
            takenMedicines[medNo].quantity = medqty;
            console.log(takenMedicines);
            let subt = document.getElementById("subtotal").innerHTML;
            subt = Number(subt) - (oldqty * medprice);
            subt = Number(subt) + (medqty * medprice);
        ...
        const removeRow = (rowid) => {
            ...
            const medprice = document.getElementById(`row-${rowid}`).childNodes[2].innerHTML;
            let subt = document.getElementById("subtotal").innerHTML;
        ...
        const selectThis = async () => {
            ...
            if(quantity > qty){
                alert(`${medicine} is not sufficient in stock\nPlease reduce the quantity\nCurrent stock is ${qty}`);
                let quantity = document.getElementById('medicine').nextElementSibling.value = 0;
                return;
            }
            console.log(price);
            ...
            // console
            document.getElementById('subtotal').innerHTML = Number(price * quantity) + Number(old);
            syncTotal();
        ...
        const showAlternative = (data) => {
            ...
            $.each(data, function (key, value) {
                ...
                $("#alternatives").append(`<div class="showalts"><strong>${key}</strong> - price: ${value[1]} Quantity: ${value[0]} <span onclick="choosethis('${key}')"><img src = "/static/check.png" height="30px"></span></div>`);
            });
        }
